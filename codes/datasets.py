...
class ImageDataset(Dataset):
    ...
    def __init__(self, root, split='train', mode='fine', augment=False):
        ...
        self.mapping = {
            ...
            20: 0,  # traffic sign
            21: 0,  # vegetation
            22: 0,  # terrainExpected object of scalar type Long but got scalar type Byte for argument #2 'target'
            23: 2,  # sky
            24: 6,  # person
            25: 0,  # rider
            26: 3,  # car
        ...
        self.mappingrgb = {
            ...
            4: (255, 0, 0),  # static
            5: (255, 0, 0),  # dynamic
            6: (255, 0, 0),  # groundExpected object of scalar type Long but got scalar type Byte for argument #2 'target'
            7: (0, 255, 0),  # road
            8: (255, 0, 0),  # sidewalk
            9: (255, 0, 0),  # parking
    ...
    def __getitem__(self, index):
        ...
        dep_target_RGB = Image.open(self.targets_depth[index]).convert('L')
        ...
        if self.augment:
            ...
            dep_target_RGB = TF.resize(dep_target_RGB, size=(128+10, 256+10), interpolation=Image.NEAREST)
            ...
            dep_target_RGB = TF.crop(dep_target_RGB, i, j, h, w)
            ...
            if random.random() > 0.5:
                ...
                dep_target_RGB = TF.hflip(dep_target_RGB)
            ...
            dep_target_RGB = TF.resize(dep_target_RGB, size=(128, 256), interpolation=Image.BILINEAR)
        ...
        dep_target = torch.from_numpy(np.array(dep_target, dtype=np.float))
        dep_target_RGB = torch.from_numpy(np.array(dep_target_RGB, dtype=np.uint8))
        image = TF.to_tensor(image)

        # convert the labels into a mask
        targetrgb = self.mask_to_rgb(sem_target)        
        targetmask = self.mask_to_class(sem_target)
        targetmask = targetmask.long()
        targetrgb = targetrgb.long()
        dep_target = dep_target.long()
        dep_target_RGB = dep_target_RGB.permute(2,1,0)
        dep_target_RGB = dep_target_RGB.long()
       

        # finally return the image pair
        return image, targetmask, targetrgb, dep_target, dep_target_RGB
